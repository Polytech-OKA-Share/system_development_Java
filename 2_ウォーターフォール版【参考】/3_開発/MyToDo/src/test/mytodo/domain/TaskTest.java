...
class TaskTest {
	...
	void completeTask() {
		...
		// テスト用のタスクを完了状態にする
		t.complete();
		// ステータスが完了(Status.DONE)になっているか検証する
		assertEquals(Status.DONE, t.getStatus(), "ステータスが未完了のまま");
	}
	
	@Test
    @DisplayName("タスクの詳細を更新できる")
	void updateDetails() {
		LocalDateTime dueDate = LocalDateTime.of(2026, 12, 31, 0, 0);
		// 旧タスク
		Task task = new Task("旧タイトル", "旧内容", dueDate, Priority.LOW);
        // 旧タスクの内容を更新
		task.updateDetails("新タイトル", "新内容", dueDate, Priority.HIGH);
		// 内容が更新されているか検証
		assertAll(
