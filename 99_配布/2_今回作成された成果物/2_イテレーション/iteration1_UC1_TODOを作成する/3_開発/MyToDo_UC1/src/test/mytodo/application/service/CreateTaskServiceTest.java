...
class CreateTaskServiceTest {
	...
	@Test
	@DisplayName("期限が空のとき例外が投げられsave()は呼ばれない")
	void execute_throwsWhenDueDatePast_andDoesNotSave() {
		// Arrange:テストで利用する不正なコマンドを準備
		LocalDateTime past = LocalDateTime.of(2025, 12, 31, 0, 0);
		CreateCommand command = new CreateCommand("", "内容", past, Priority.LOW);
		// Act:不正コマンドで実行。例外を投げるためその処理全体を変数に格納
		Executable act = () -> service.execute(command);
		// Assert:actの処理の流れで例外が投げられることを確認
		assertThrows(IllegalArgumentException.class, act);
